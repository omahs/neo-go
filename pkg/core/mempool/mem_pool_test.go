...
func TestMempoolAddWithDataGetData(t *testing.T) {
	...
	r1 := &payload.P2PNotaryRequest{
		MainTransaction:     newTx(0, &nonce),
		FallbackTransaction: newTx(fs.balance+1, &nonce),
	}
	...
	r2 := &payload.P2PNotaryRequest{
		MainTransaction:     newTx(0, &nonce),
		FallbackTransaction: newTx(smallNetFee, &nonce),
	}
	...
	r3 := &payload.P2PNotaryRequest{
		MainTransaction:     newTx(0, &nonce),
		FallbackTransaction: newTx(smallNetFee+1, &nonce),
	}
...
func newTx(netFee int64, nonce *uint32) *transaction.Transaction {
